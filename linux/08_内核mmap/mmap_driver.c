...
#define MAJOR_NUM 490 //990太大，超过512  
#define MM_SIZE 4096  
...
{  
    printk(KERN_ALERT"mmap device open\n");  
...
{  
	...
	//https://blog.csdn.net/weixin_42096901/article/details/103228714
	//标志该内存区不能被换出，在设备驱动中虚拟页和物理页的关系应该是长期的，应该保留起来，不能随便被别的虚拟页换出 
    //vma->vm_flags |= VM_RESERVED; //error 
...
{  
    ...
    printk(KERN_ALERT"mmap module init start\n");  
    printk("mmap dev=%d", dev);  
    dev = MKDEV(dev_major, dev_minor);  
    cdev = cdev_alloc();//为字符设备cdev分配空间  
    printk(KERN_ALERT"mmap module init\n");  
    if(dev_major)  
    {  
        result = register_chrdev_region(dev, 1, driver_name);//静态分配设备号  
        printk("mmap static region result = %d\n", result);  
    }  
    else  
    {  
    ...
    if(result < 0)  
    {  
        printk(KERN_WARNING"mmap Cant't get major %d\n", dev_major);  
        return result;  
    }  
...
{  
    printk(KERN_ALERT"mmap module exit\n");  
    cdev_del(cdev);  
